<template>
  <div class="mobile-container mobile-input-money"
       @click="select(id,index)"
       v-bind:style="style">
    <div class="mobile-content input">
      <h1>{{contents.title}}</h1><div class="ipt"><i class="iconfont icon-anonymous-iconfont" :class="{'icon-anonymous-iconfont':contents.unit  == 1,'icon-renminbi':contents.unit == 2 }" ></i><input type="number" :placeholder="contents.placeholder" v-model.number="contents.value"></div>
    </div>
  </div>
</template>
...
<style scoped>
  ...
  .mobile-input-money>.input>.ipt>i{
    display: block;
    height: 100%;
    width: auto;
    left: .15rem;
    position: absolute;
    color: rgba(110, 110, 110, 0.53);
    font-size: .16rem;
    vertical-align: middle;

  }
  .mobile-input-money>.input>.ipt>input{
    display: block;
    height: 100%;
    width: 100%;
