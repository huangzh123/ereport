<template>
  <div class="mobile-container mobile-input-text"
       @click="select(id,index)"
       v-bind:style="style">
    <div class="mobile-content input">
      <h1>{{contents.title}}</h1><div class="ipt"><input type="text" :placeholder="contents.placeholder" v-model="contents.value" ></div>
    </div>
  </div>
</template>
...
<style scoped>
  ...
  .mobile-input-text>.input>.ipt>input{
    display: block;
    height: 100%;
    width: 100%;
