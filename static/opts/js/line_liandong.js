  ...
  opts: [{
      ...
      members: [{
        ...
        },
        {
          ...
          describe: '',
          remark: '',
          model: "contents.series[0].areaStyle.color",
          component: 'webInputColor'
        },
        {
          title: '边线颜色',
      ...
      members: [{
        ...
        },
        {
          ...
          init(opt, cb) {
            ...
            if (opt.others.linkArrs) {
              let v = JSON.parse(JSON.stringify(opt.others.linkArrs));
              self.$options.methods.triggerMethod(self, 'self.$options.methods.disconnectCharts(self)');
              v.push(opt.cid + "_dump");
              let str = 'self.$options.methods.connectCharts(self,"' + v + '")';
              self.$options.methods.triggerMethod(self, str);
              if (cb) cb();
            }
          },
          updateValue(opt, cb) {
            let self = opt.self;
            let setting = opt.setting;
            let v = JSON.parse(JSON.stringify(opt.value));
            self.$options.methods.triggerMethod(self, 'self.$options.methods.disconnectCharts(self)');
